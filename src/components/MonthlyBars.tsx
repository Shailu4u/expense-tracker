  ...
  return (
    <View>
      <Svg width={width} height={height}>
        <Line x1={padding} y1={height - padding} x2={width - padding} y2={height - padding} stroke={palette.outlineVariant} strokeWidth={1} />
        {data.map((d, i) => {
          ...
          return (
            <G key={d.label}>
              <Rect x={x} y={height - padding - incH} width={(groupWidth - barGap) / 2} height={incH} rx={2} fill={palette.primaryFixedDim} />
              <Rect x={x + (groupWidth + barGap) / 2} y={height - padding - expH} width={(groupWidth - barGap) / 2} height={expH} rx={2} fill={palette.primary} />
            </G>
          );
        })}
      </Svg>
      <View style={styles.labels}>
        {data.map((d) => (
          <ThemedText key={d.label} variant="labelCaps" tone="muted" style={styles.label}>{d.label}</ThemedText>
        ))}
      </View>
      <View style={styles.legend}>
        <Legend color={palette.primaryFixedDim} label="Income" />
        <Legend color={palette.primary} label="Expense" />
        <ThemedText variant="bodySm" tone="muted" style={{ marginLeft: 'auto' }}>Max {formatINR(max)}</ThemedText>
      </View>
    </View>
  );
}
...
function Legend({ color, label }: { color: string; label: string }) {
  return (
    <View style={styles.legendItem}>
      ...
      <ThemedText variant="bodySm" tone="muted">{label}</ThemedText>
    </View>
  );
}
